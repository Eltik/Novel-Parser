...
import AniList, { Type, Media, Format } from "./AniList";
...
export default class Core extends API {
    ...
    public async search(query:string): Promise<FormattedResponse[]> {
        ...
        const aniSearch = await this.aniSearch(query, Type.MANGA);
        if (this.config.debug) {
        ...
        const pageSearch = await this.pageSearch(query, Type.MANGA);
        if (this.config.debug) {
